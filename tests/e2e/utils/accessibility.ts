 ...
 */
export async function checkA11yRule(page: Page, ruleId: string) {
 ...
 * Runs axe ARIA rules
 */
export async function checkARIA(page: Page) {
  const results = await new AxeBuilder({ page })
...
export async function checkHeadingHierarchy(page: Page) {
  ...
  for (let i = 1; i < levels.length; i++) {
    ...
    expect(
      diff,
      `Heading hierarchy violation: h${levels[i - 1]} followed by h${levels[i]}`
 ...
 */
export async function checkFocusVisibility(page: Page) {
  const interactiveElements = await page.locator(
