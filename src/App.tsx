 ...
 */
function formatCriteriaDetails(
  ...
  return criteriaResults.map(cr => {
    ...
    // Fallback to more context-aware descriptions
    if (isEligible) {
      return `${fieldName}: Meets requirements`;
    } else {
      // For failed cases, be more specific about what it means
      if (cr.met) {
        return `${fieldName}: Data verified but contributes to ineligibility`;
      } else {
